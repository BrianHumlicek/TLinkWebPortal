...
namespace DSC.TLink.ITv2
{
	internal partial class ITv2Session
	{
		...
		class SimpleAckTransaction : Transaction
		{
			...
			protected override async Task ContinueAsync(ITv2Message message, CancellationToken cancellationToken)
			{
				switch (_state)
				{
					case State.AwaitingSimpleAck:
						// We sent a message, expecting SimpleAck back
            ...
            {
                if (_state == State.AwaitingSimpleAck && message.messageData is not SimpleAck)
                {
			...
			private enum State
			{
				...
				AwaitingSimpleAck,    // Outbound: waiting for remote's ack
				Complete
			}
		}
	}
